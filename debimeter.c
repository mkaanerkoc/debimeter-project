...
#define  FILE_LOG_HEADER    "Saat; V_QP; V_QPF; V_AVG; V_RAW; Temperature; Humidity; Pressure; Amplitude; Level; FlowRate; Total Flow; DailyFlow;  Milivolt; Miliampere; Height ( mm )"
#define  FILE_LOG_HEADER_SIZE strlen( FILE_LOG_HEADER )


uint16_t                    modbusAsciiRegisterArrayTemp[ 30 ];
...
totalVolumeManager			totalVm;
totalVolumeManager 			dailyVm;
...
int main( void )
{
    ...
    else
    {
		...
		init_totalVolumeManager(&totalVm,"total_vm");
		init_totalVolumeManager(&dailyVm,"daily_vm");
    ...
    serialPortResultTemp = SerialPort_Begin( appConfigSt.serialPortName , B19200, 8, 0 , 2 );
    ...
    printf("App started succesfully\n");
    BeginMainTask();
    RemoteControl_Spawn();
...
void *DeviceMainTask(void *arg)
{
	...
	struct tm           *dailyVmLastResetCheck,*dailyVmLastResetControl;
    ...
    time_t   			timeStampHolder; // usendim bunu yapmaya uff
    while( 1 )
    {
        ...
        if( true == deviceReadPeriodOccured )
        {
            ...
            if( 0 == appConfigSt.useNotchFormula )
            {
                ...
                modbusResultTemp = Read_RavenEye_Values( &ravenEyeInfo ); // read raven eye values    flowRate m3/day burada,
        		...
        		//dailyVmLastResetCheck = localtime( &(dailyVm.lastReset));
        		dailyVmLastResetControl = gmtime( &timeStampHolder );
                //printf("dailyVmLastResetControl.day %d , dailyVmLastResetCheck : %d \n", dailyVmLastResetControl->tm_mday, dailyVmLastResetCheck->tm_mday );
        		if( dailyVmLastResetControl->tm_hour < (( dailyVm.lastReset % 86400 ) / 3600) )
        		{
                    printf("Daily Totalizor resetted ! \n");
        			dailyVm.ResetValue(&dailyVm);
        		}
        		ravenEyeInfo.totalFlowRate_Q = totalVm.value;
        		ravenEyeInfo.dailyTotalFlowRate_Q = dailyVm.value;
            ...
            {
                float flowRate_Temp = Calculate_FlowRateWithNotchFormula( ads1115Info.height, appConfigSt.notchWidth ); // buradaki flowRateBirimi ne ?
        		...
        		/*printf("dailyVmLastResetControl : %02d:%02d:%02d , dailyVmLastResetCheck : %02d:%02d:%02d , dailyVm.lastReset :  %02d:%02d:%02d \n", 
                    ...
                    dailyVmLastResetCheck->tm_hour, dailyVmLastResetCheck->tm_min,dailyVmLastResetCheck->tm_sec,
                    ( dailyVm.lastReset % 86400 ) / 3600, (( dailyVm.lastReset % 86400 ) % 3600 ) / 60, ( dailyVm.lastReset % 86400 ) % 60  );*/
                if( dailyVmLastResetControl->tm_hour < (( dailyVm.lastReset % 86400 ) / 3600) )
        		{
                    printf("Daily Totalizor resetted  tm_hour : %d , lastReset : %d ! \n", dailyVmLastResetControl->tm_hour, (( dailyVm.lastReset % 86400 ) / 3600) );
        			dailyVm.ResetValue(&dailyVm);
        		}
...
void Set_RavenEye_ValuesOnModbusTCP( RavenEyenRegisterStruct *ravenEyeInfo, uint16_t *writtenRegisterCount )
{
    ...
    mbusRegister.fvalue = ravenEyeInfo->totalFlowRate_Q;
    Modbus_SetRegisterValue( registerCountTemp++ ,INPUT_REGISTER_TYPE, mbusRegister.words[0] );
    Modbus_SetRegisterValue( registerCountTemp++ ,INPUT_REGISTER_TYPE, mbusRegister.words[1] );
		
	mbusRegister.fvalue = ravenEyeInfo->dailyTotalFlowRate_Q;
    Modbus_SetRegisterValue( registerCountTemp++ ,INPUT_REGISTER_TYPE, mbusRegister.words[0] );
    Modbus_SetRegisterValue( registerCountTemp++ ,INPUT_REGISTER_TYPE, mbusRegister.words[1] );
...
void Send_RavenEye_ValuesToRemoteController( RavenEyenRegisterStruct *ravenEyeInfo )
{
    ...
    DpArr[ ravenEyeDataIndex++ ] = Dp;
 	...
 	Dp.data.floatData = ravenEyeInfo->totalFlowRate_Q;
    Dp.id = TOTAL_VOLUME_FLOW;
    DpArr[ ravenEyeDataIndex++ ] = Dp;

	Dp.data.u64Data = totalVm.lastReset;
    Dp.type = 2;
    Dp.id = LAST_TOTAL_VOLUME_RESET;
    DpArr[ ravenEyeDataIndex++ ] = Dp;

    Dp.type = 1;
	Dp.data.floatData = ravenEyeInfo->dailyTotalFlowRate_Q;
    Dp.id = DAILY_VOLUME_FLOW;
    DpArr[ ravenEyeDataIndex++ ] = Dp;

    time_t lastTime;
    time(&lastTime);
    Dp.data.u64Data = lastTime;
    Dp.type = 2;
    Dp.id = 200;
    DpArr[ ravenEyeDataIndex++ ] = Dp;
...
void Get_RavenEye_ValuesToDatabaseBuffer( RavenEyenRegisterStruct *ravenEyeInfo, float *dbBuffer, uint8_t *count )
{
	...
	dbBuffer[ countTemp++ ] = ravenEyeInfo->totalFlowRate_Q;
	dbBuffer[ countTemp++ ] = ravenEyeInfo->dailyTotalFlowRate_Q;
    *(count) = countTemp;   
}
